...
function updateCarousel() {
    ...
    btnNext.disabled = currentIndex === productImages.length;
}
...
const itemWidth = 68; // ширина превью + отступ
...
productImages.forEach((src, index) => {
    ...
    img.addEventListener('click', () => {
        previewContainer.querySelector('.preview-selected')?.classList.remove('preview-selected');
...
btnNext.addEventListener('click', () => {
    const currentImg = previewContainer.querySelector('.preview-selected');
    let nextImg = currentImg.nextElementSibling;
    if (!nextImg) {
        nextImg = previewContainer.firstElementChild;
    }
    currentImg.classList.remove('preview-selected');
    nextImg.classList.add('preview-selected');
    imageMain.src = nextImg.src;
...
btnPrev.addEventListener('click', () => {
    ...
    if (!previousImg) {
        previousImg = previewContainer.lastElementChild;
    }
    currentImg.classList.remove('preview-selected');
    previousImg.classList.add('preview-selected');
    imageMain.src = previousImg.src;
